---
description: >-
  Steps to transfer/wrap SRI20 tokens from Ethereum to SRInet and back using
  smart contracts.
---

# Using the bridge with SRI20 tokens directly through the contract

**Please use this tutorial at your own risk as it involves using Etherscan UI/SRI Blockchain explorer to relay the tokens. This tutorial is applicable only for SRI20 tokens other than SRI. Do not use this tutorial to transfer SRI.** 
...
SRInet Mediator : **0xc2220646E1E76D5fF3a441eDd9E8EFF0e4A8EF03**

In the below example we will consider USDT \(SRI20\) and learn how to wrap USDT to SRInet and relay it back to SRI20.
...
Once the transaction is confirmed on-chain we wait for 2 blocks to ensure security of transaction and then the USDT tokens should appear on your SRI Blockchain address and have been swapped from Ethereum mainnet to SRInet.

Now let’s learn how to transfer the wrapped SRI20 tokens on SRInet back to Ethereum mainnet.

**Step 1:** Approving the wrapped USDT token contract on SRInet. 

This step basically is to approve your wallet to interact with the USDT token contract on SRInet so that you can transfer the tokens to the Mediator contract.

Please go to token contract on SRInet explorer via link below.

https://poolscan.io/address/0xFaDbBF8Ce7D5b7041bE672561bbA99f79c532e10/write\_proxy

![](../../.gitbook/assets/4%20%286%29.png)

Make sure that the network is SRI network. If you have not added SRI Blockchain network please follow the instructions [here](https://doc.poolscan.io/the-SRI-studio/getting-started/how-to-add-SRI-to-your-metamask).

Click on **“Connect to Metamask”** and sign in to your wallet through Metamask.

 **Step 2:** Go to **“approve”** and enter the details below

* spender \(address\) field: the mediator contract address on SRInet \(0xc2220646E1E76D5fF3a441eDd9E8EFF0e4A8EF03\)
...
Navigate to Migrator contract on SRInet using the link below

[https://poolscan.io/address/0xc2220646E1E76D5fF3a441eDd9E8EFF0e4A8EF03/write\_proxy](https://poolscan.io/address/0xc2220646E1E76D5fF3a441eDd9E8EFF0e4A8EF03/write_proxy)
...
* token \(address\) field: the USDT token contract address on SRInet \(0xFaDbBF8Ce7D5b7041bE672561bbA99f79c532e10\)
* \_value \(uint256\): the amount of tokens to transfer in wei \(Number of decimals should be 6\)

![](../../.gitbook/assets/6%20%285%29.png)

After the transaction is confirmed on SRI Blockchain network, the bridge oracle will relay your tx on Ethereum. No need to wait for additional confirmations as SRI Blockchain is PoS network. After sometime you should be able to see the successful transfer of the token on your SRI20 address.
